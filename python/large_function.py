"""
Python test file with a large function that has been refactored.
...
def process_and_analyze_data(
    ...
    try:
        ...
        data, filter_stats = apply_filtering(data, filter_criteria)
        results.update(filter_stats)
        
        # Step 6: Calculate aggregations
        aggregations = calculate_aggregations(data, aggregation_fields)
        if aggregations:
            results["aggregations"] = aggregations
        
        # Step 7: Compute overall statistics
        statistics = compute_overall_statistics(data, include_stats)
        ...
        write_output_file(output_path, output_data, output_format)
